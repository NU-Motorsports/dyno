...
void print_data_point(data_point *dp) {
  printf("prrpm:\t%" PRIu16 "\tserpm:\t%" PRIu16 "\ttrque:\t%" PRIu16 "\n"
         ...
         "t_p_s:\t%" PRIu16 "\tbi_sp: %6.2f"
         "\ttpssp: %6.2f"
         "\n",
         dp->prim_rpm, dp->sec_rpm, dp->torque, dp->temp3, dp->belt_temp,
         dp->temp2, dp->i_brake, dp->temp1, dp->load_cell, dp->tps, dp->i_sp,
         dp->tps_sp);
}
...
static void write_logging_queue_to_sd(void *arg) {
  ...
  // per FreeRTOS, tasks MUST be deleted before breaking out of its implementing
  // funciton also release mutex
