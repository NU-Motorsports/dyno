...
xQueueHandle logging_queue_1, logging_queue_2,
    current_dp_queue; // queues to store data points
...
static void daq_timer_init() {
  ...
  timer_set_alarm_value(DAQ_TIMER_GROUP, DAQ_TIMER_IDX,
                        TIMER_BASE_CLK / DAQ_TIMER_DIVIDER / DAQ_TIMER_HZ);
  timer_enable_intr(DAQ_TIMER_GROUP, DAQ_TIMER_IDX);
  timer_isr_register(DAQ_TIMER_GROUP, DAQ_TIMER_IDX, daq_timer_isr, NULL,
...
static void get_profile() {
  ...
  case 4:
    for (i = 0; i < BSIZE; i++) {
      ...
      tps_sp[i] = 0.05 * BREAK_IN_RPM -
                  90; // variable throttle disabled; constant value
    }
    break;
...
static void daq_task(void *arg) {
  ...
  data_point_t dp = {
      ...
      dp.tps = 0,      dp.i_sp = 0,      dp.tps_sp = 0}; // empty data point

  // module, peripheral configurations

  // init ADC w/ channel selection
  i2c_master_config(PORT_0, FAST_MODE_PLUS, I2C_MASTER_0_SDA_IO,
                    I2C_MASTER_0_SCL_IO);
  ...
  // init PIDs
  init_pid(&brake_current_pid, KP, KI, KD, BRAKE_WINDUP_GUARD,
           BRAKE_OUTPUT_MAX);
  ...
  while (main_ctrl.run) {
    ...
    if (main_ctrl.en_log) {
      // RECORD DATA
      // adc
      ad7998_read_3(PORT_0, ADC_SLAVE_ADDR, &(dp.torque), &(dp.temp3),
                    &(dp.belt_temp), &(dp.temp2), &(dp.i_brake), &(dp.temp1),
                    &(dp.load_cell), &(dp.tps));

      // relevant physical quantity conversion for faults
      // ADC counts to amps
      main_ctrl.i_brake_amps =
          (counts_to_volts(dp.i_brake) * I_BRAKE_SCALE) + I_BRAKE_OFFSET;

      // convert brake current in amps to duty cycle from 0-100%
      main_ctrl.i_brake_duty = 100 * (main_ctrl.i_brake_amps / I_BRAKE_MAX);

      // ADC counts to amps
      main_ctrl.brake_temp =
          (counts_to_volts(dp.temp3) * THERM_SCALE) + THERM_OFFSET;

      // ADC counts to amps
      main_ctrl.belt_temp =
          (counts_to_volts(dp.belt_temp) * BELT_TEMP_SCALE) + BELT_TEMP_OFFSET;
    ...
    if ((main_ctrl.belt_temp > MAX_BELT_TEMP) |
        (main_ctrl.brake_temp > MAX_BRAKE_TEMP)) {
      ctrl_faults.trip = 1;
    ...
    if (main_ctrl.en_log) {
      if (xQueueSend(current_logging_queue, &dp, 0) == errQUEUE_FULL) {
        ...
        if (current_logging_queue == logging_queue_1) {
          ...
          xTaskCreatePinnedToCore(write_logging_queue_to_sd, "write_lq_1_sd",
                                  2048, (void *)logging_queue_1,
                                  (configMAX_PRIORITIES - 1), NULL, 1);
        } else {
          current_logging_queue = logging_queue_1;
          xTaskCreatePinnedToCore(write_logging_queue_to_sd, "write_lq_2_sd",
                                  2048, (void *)logging_queue_2,
                                  (configMAX_PRIORITIES - 1), NULL, 1);
        }
  ...
  flasher_off();
  ebrake_set();
  xTaskCreatePinnedToCore(write_logging_queue_to_sd, "write_lq_final_sd", 2048,
                          (void *)current_logging_queue,
                          (configMAX_PRIORITIES - 1), NULL, 1);

  // per FreeRTOS, tasks MUST be deleted before breaking out of its
  // implementing funciton
  vTaskDelete(NULL);
}
...
extern "C" void app_main(void) {
  ...
  xTaskCreatePinnedToCore(daq_task, "daq_task", 4096, NULL,
