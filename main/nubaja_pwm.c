...
// width varies between 1000 to 2000 mocrosecond
...
#define SERVO_MAX_DEGREE     120  // Maximum angle (deg) which servo can rotate
...
void pwm_init() {
  ...
  pwm_config.duty_mode = MCPWM_DUTY_MODE_1; // inverted
...
void set_throttle(float tp) {
  float servo_angle =
      (tp * 0.9) + SERVO_OFFSET_DEG; // TO-DO - DEFINE THIS RELATIONSHIP
